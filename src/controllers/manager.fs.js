  ...
  async updateById(id, params) {
    if (fs.existsSync(this._pathToFile)) {
      ...
      if (productoId !== -1) {
        ...
        return {
          status: "success",
          mensaje: `El producto con el id (${id}) fue modificado`,
          productos,
        };
      }
      return {
        status: "error",
        mensaje: `No existe producto con el id (${id})`,
      };
    } else {
  ...
  async deleteById(id) {
    ...
    if (fs.existsSync(this._pathToFile)) {
      ...
      if (validation) {
        ...
        await fs.promises.writeFile(
          this._pathToFile,
          JSON.stringify(newProductosArray, null, 2)
        );
        if (newProductosArray.length === 0) {
          return {
            status: "success",
            mensaje: "Producto Eliminado, no hay mas productos",
          };
        }

...
export const validationBody = (req, res, next) => {
  ...
  if (!nombre || !descripcion || !codigo || !foto || !precio || !stock) {
    return res
      .status(400)
      .send({ error: -2, descripcion: `Todos los campos son obligatorios` });
  }
  next();
};
...
const createDBLocal = async (req, res, next) => {
  ...
  connectDB.connect(function (err, result) {
    if (err) throw err;
    connectDB.query(
      "CREATE DATABASE IF NOT EXISTS websocket_mariadb",
      function (err, result) {
...
const midProducts = (req, res, next) => {
  ...
  if (!title || !price || !thumbnail)
    return res.status(400).send({ err: "This data is required!" });
  next();
};
